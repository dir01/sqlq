...
type JobsQueue interface {
	...
	PublishTx(ctx context.Context, tx *sql.Tx, jobType string, payload any) error
	Subscribe(ctx context.Context, jobType string, consumerName string, f func(ctx context.Context, payloadBytes []byte) error)
	Shutdown()
	Run()
}
...
func (q *SQLQueue) Publish(ctx context.Context, jobType string, payload any) error {
	tx, err := q.db.BeginTx(ctx, nil)
	if err != nil {
		return fmt.Errorf("failed to begin transaction: %w", err)
	}

	if err := q.PublishTx(ctx, tx, jobType, payload); err != nil {
		tx.Rollback()
		return err
	}
...
func (q *SQLQueue) PublishTx(ctx context.Context, tx *sql.Tx, jobType string, payload any) error {
	...
	params := q.driver.FormatQueryParams(jobType, payloadBytes)
...
func (q *SQLQueue) workerLoop(sub *subscriber) {
	...
	for {
		...
		case job, ok := <-sub.processingJobs:
			...
			// Process the job
			if err := sub.handler(sub.ctx, tx, job.Payload); err != nil {
