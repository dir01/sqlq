...
func (d *SQLiteDriver) InsertJob(
	...
	if delay <= 0 {
		...
		// For delayed jobs, calculate the scheduled time using SQLite's datetime functions
...
func (d *SQLiteDriver) GetJobsForConsumer(ctx context.Context, consumerName, jobType string, prefetchCount int) ([]job, error) {
	...
	rows, err := d.db.QueryContext(ctx, `
		...
		WHERE j.job_type = ? 
		AND j.scheduled_at <= (strftime('%s','now') * 1000)
...
func (d *SQLiteDriver) MarkJobProcessed(ctx context.Context, jobID int64, consumerName string) error {
	...
	_, err := d.db.ExecContext(ctx,
		"INSERT INTO job_consumers (job_id, consumer_name, processed_at) VALUES (?, ?, (strftime('%s','now') * 1000))",
		jobID, consumerName,
	)
	if err != nil {
		span.RecordError(err)
...
func (d *SQLiteDriver) MarkJobFailedAndReschedule(
	...
	_, err := d.db.ExecContext(ctx,
		`UPDATE jobs SET 
			...
			last_error = ?, 
			scheduled_at = (strftime('%s','now', '+' || ? || ' seconds') * 1000) 
		WHERE id = ?`,
